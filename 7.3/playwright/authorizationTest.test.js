...
const { test, expect } = require('@playwright/test');//  указаваем какой браузер и библиотеку используем
const user = require('../user'); // указываем где барать доп. инфу браузеру

test.beforeEach(async ({ page }) => { // настройка предварительных условий
  await page.goto('https://netology.ru/?modal=sign_in'); // открываем старницу
});

test.afterEach(async ({ page }) => { // настройка предварительных условий
